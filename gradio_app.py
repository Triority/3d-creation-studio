                ...
                with gr.Tabs(selected='tab_img_prompt') as tabs_prompt:
                    ...
                    with gr.Tab('Text Prompt', id='tab_txt_prompt', visible=HAS_T2I and not MV_MODE) as tab_tp:
                        caption = gr.Textbox(label='Text Prompt',
                                             placeholder='A studio product photo of a complete object on a white background')
...
if __name__ == '__main__':
    ...
    if not args.disable_tex:
        try:
            ...
            conf.dino_ckpt_path = "/media/B/Triority/Hunyuan3D-2.1/models/dinov2-giant"
    ...
    if args.enable_t23d:
        ...
        t2i_pipeline = HunyuanDiTPipeline.from_pretrained(
            "/media/B/Triority/Hunyuan3D-2.1/models/HunyuanDiT",
            torch_dtype=torch.float16,
        )
        t2i_pipeline.enable_model_cpu_offload()

        def t2i_worker(prompt):
            prompt = f"{prompt}, isolated complete object, centered, white background, studio lighting"
            return t2i_pipeline(
                prompt=prompt,
                height=1024,
                width=1024,
